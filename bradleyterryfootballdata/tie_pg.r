...
PGSmootherPost = function(samples_size, iterations, thinning, burn_in, win_vector_matrix, tie_matrix, total_matrix, pho, sigma, index_matrix_list){
  start.time = Sys.time()
  print("Start time = ")
  print(start.time)
    ...
    for (t in 1 : time){
      for (i in 1 : team_pair_length){
        ...
        if (total_matrix[i,t] == 0){
          ...
          poly_gamma_var_2[i, t] = rpg(1, s_ji_total_rev + t_ij_total, team_play_matrix[i, t]/2)
        }
      }
    }
